...
class MCNFPDHGWARMSTART:
    ...
    def generate_initial_flow_gpu(self,scale_factor=0.5):
        """
        ...
        x_flow=x_flow*scale_factor
                
        # 初始化矩阵形式对偶变量Y_matrix(N, K)：对于商品k，当前节点距离k_dst越远，则Y值越大，令其流向k_dst
        Y_matrix=D[:,k_dst]
        # 替换Y中inf值防止PDHG梯度出现NaN
        finite_mask = torch.isfinite(Y_matrix)
        if finite_mask.any():
            max_dist = Y_matrix[finite_mask].max()
            # 替换 inf 为 max_dist * 2.0，保持很远但数值稳定
            fill_value = max_dist * 2.0
            Y_matrix = torch.where(finite_mask, Y_matrix, fill_value)
        else:
            # 极端情况：全是 inf (图完全不连通)，全设为 0
            Y_matrix = torch.zeros_like(Y_matrix)

        return x_flow.reshape(-1),self.d.clone()*scale_factor,Y_matrix.reshape(-1)
