...
def calculate_objective(x, X, w, d, p):
    obj = w @ torch.square(X - d) + p @ torch.sum(x.reshape(M, K), dim=1)
    return obj
...
# print('求解器GUROBI结果')
# print(f"求解状态: {result_gurobi['status']}")
# print(f"使用求解器: {result_gurobi['solver']}")
# print(f"求解器内置求解时间: {result_gurobi['solve_time']}")
